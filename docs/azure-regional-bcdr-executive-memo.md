# Executive Memo: Azure BCDR and Relocation Options for a US East 2 Deployment
...
A US East 2-only Azure design is not sufficient if the threat model includes prolonged loss of US East 2 or a broader East Coast disruption. Azure availability zones and paired-region concepts improve resiliency, but they do not by themselves provide a complete hedge against a wide-area operating disruption. The customer should plan for three separate failure scopes:
...
- Appropriate for customers who want a balanced model with paired-region benefits and lower operational complexity.
...
South Central US can be a reasonable compromise when the customer wants US-based separation with lower latency than a far-west design, but it does not provide the same paired-region benefits as Central US.
...
This improves resilience to a single-region event and aligns with Azure paired-region guidance, but it remains weaker than an east-plus-west design for a wider East Coast disruption.

## Decision Guidance

### If resilience is top priority

Use a recovery region outside the East Coast immediately and evaluate whether a western secondary region is required in addition to Central US.
...
Use West US 2 or West US 3 as the secondary geography and document the cost, latency, and operating tradeoff relative to Central US.
...
4. Fund a real disaster recovery environment rather than relying only on backups.
5. Test failover and executive decision processes regularly.

## Key Message for Leadership

If the planning assumption is that US East 2 or the broader East Coast could become unavailable as an operating environment, then another zone in US East 2 is not enough. The appropriate hedge is another geography.
